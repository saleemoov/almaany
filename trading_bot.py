...
from config import (
    ...
    OKX_API_KEY, OKX_SECRET, OKX_PASSWORD, ALERT_COOLDOWN_HOURS
)
...
class TradingBot:
    ...
    async def process_coin(self, coin, open_trades):
        ...
        if not self.can_send_alert(coin):
            logger.info(f"Skipping {coin} - alert cooldown active")
            return
    ...
    async def update_open_trades(self):
        ...
        for trade in open_trades:
            ...
            if current_price >= trade['tp3_price'] and trade.get('sold_tp3', 0) == 0:
                remaining_qty = trade['quantity'] * (1 - trade.get('sold_tp1', 0) - trade.get('sold_tp2', 0))
    ...
    async def partial_close_trade(self, trade, sell_qty, price, reason):
        ...
        try:
            await self.exchange.create_market_sell_order(symbol, sell_qty)
    ...
    async def close_trade(self, trade, price, reason):
        ...
        if remaining_qty > 0:
            symbol = f"{trade['coin']}/USDT"
            try:
                await self.exchange.create_market_sell_order(symbol, remaining_qty)
...
[4/13/2026 12:01 AM] Saleemmajed: pnl = (price - trade['entry_price']) * trade['quantity']
    ...
    async def send_status_report(self):
        ...
        market_condition = Counter(conditions).most_common(1)[0][0] if conditions else 'UNKNOWN'
    ...
    async def send_weekly_report(self):
        ...
        weekly_trades = [
            ...
            if t['exit_time'] and datetime.fromisoformat(t['exit_time']) > week_ago and t['status'] == 'closed'
        ]
